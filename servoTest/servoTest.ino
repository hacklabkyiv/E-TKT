// ~~~~~~IMPORTANT: do not forget to upload the files in "data" folder via
// SPIFFS
...
Stepper stepperFeed(32, 17,  18, 5, 19);
...
#define strongAngle 9
#define lightAngle 12

// oled
U8G2_SSD1306_128X32_UNIVISION_F_HW_I2C u8g2(U8G2_R0, U8X8_PIN_NONE, 4, 16);
#define Lcd_X 128
#define Lcd_Y 64
...
const int QRcode_ECC =
  0; //  set the Error Correction level (range 0-3) or symbolic (ECC_LOW,
...
void lightChar(float state) {
  //analogWrite(ledChar, int(state * 128));
}
...
void displayInitialize() {
  ...
  u8g2.clearBuffer();
  u8g2.setFlipMode(1);
  u8g2.setContrast(1); // set OLED brightness(0->255)
  displayClear();
...
void displaySplash() {
  ...
  // animated splash
  for (int i = 128; i > -10; i -= 2) {
    u8g2.setFont(u8g2_font_inr21_mf);
    u8g2.drawStr(i, 29, "E-TKT @ HackLab");
    u8g2.sendBuffer();
    delay(1);
  }
}
...
void displayQRCode() {
  ...
  if (displayIP != "") {
    ...
    uint8_t x0 = 128 - 32;
    uint8_t y0 = 1; // 16 is the start of the blue portion OLED in the
    // yellow/blue split 64x128 OLED
...
//    return;
  ...
  sensorState = getSensorPin();

  if (!sensorState) {
    stepperChar.runToNewPosition(-stepsPerChar * 8);
    stepperChar.run();
//    Serial.println("ran");
  }
  sensorState = getSensorPin();
...
//    Serial.println("step run");
  ...
  stepperChar.runToNewPosition(stepsPerChar * homeCorrection);
  stepperChar.run();
  stepperChar.setCurrentPosition(0);
...
void feedLabel() {
  Serial.println("feedLabel()");
  stepperFeed.step(stepsPerRevolutionFeed / 8);
  delay(1000);
}
...
void clearWifiCredentials() {
  ...
  esp_wifi_init(&cfg); // initiate and allocate wifi resources (does not matter
  // if connection fails)
...
void setup() {
  ...
  //    u8g2.ssd1306_command(0x81);
  //    u8g2.ssd1306_command(dim); //max 157
  //    delay(50);
  //  }
}
